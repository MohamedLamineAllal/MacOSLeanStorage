...
func TestEngine_HookSequence(t *testing.T) {
	...
	hooks := Hooks{
		...
		OnTargetScanEnd:   func(name string, res *scanner.Result, err error) { mu.Lock(); sequence = append(sequence, "end"); mu.Unlock() },
		OnFileCleaned:     func(path string, freed int64, err error) { mu.Lock(); sequence = append(sequence, "cleaned"); mu.Unlock() },
